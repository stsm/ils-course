 ...
 Int32 fn_Division(Int32 iWert1, Int32 iWert2)
{
 ...
 Int32 fn_AlleOperationen (Int32 iWert1, Int32 iWert2)
{
	Int32 iErgebnis = 0;
	
	iErgebnis = iWert1 + iWert2;
	Console::WriteLine("Das Ergebnis von {0} + {1} ist {2}", iWert1, iWert2, iErgebnis);
...
int main(array<System::String ^> ^args)
{
	...
	do{
		...
		if ( (iOperation<=0) || (iOperation >4) ){
			...
			Console::WriteLine("Fehler! Bitte kontrollieren Sie Ihre Eingabe.");
			Console::ResetColor();
		}
		else{
			bOperation=true;
	...
	switch (iOperation){
		...
		case 4: // Division
			Console::BackgroundColor=ConsoleColor::Red;
			Console::ForegroundColor=ConsoleColor::Black;
			Console::WriteLine("Bitte beachten Sie, dass ggf. keine Nachkommastellen angezeigt werden!");
			Console::ResetColor();
	...
	Console::Write("\n");
	Console::WriteLine(" ----------------- Ergebnisse aller Operationen  ----------------- ");
	fn_AlleOperationen(iEingabe1, iEingabe2);
	Console::WriteLine(" ----------------------------------------------------------------- ");
	Console::Write("\n");
    return 0;
}